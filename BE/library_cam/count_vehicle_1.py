...
class Cam1Config:
    def __init__(self, source_path="./Data_video/Cam_1/test_faster.mp4"):
        # === DeepSORT tracker ===
        ...
        rect_w, rect_h = int(self.frame_width * 1.0), int(self.frame_height * 0.65)
        self.polygon = np.array([
            [center_x - rect_w // 2, center_y - rect_h // 2],
            [center_x + rect_w // 2, center_y - rect_h // 2],
            [center_x + rect_w // 2, center_y + rect_h // 2 + int(self.frame_height * 0.25)],
            [center_x - rect_w // 2, center_y + rect_h // 2 + int(self.frame_height * 0.25)]
        ])
        ...
        self.direction_zones = {
            "bottom": [
            (int(self.frame_width * 0.01), int(self.frame_height * 0.30)),
            (int(self.frame_width * 0.30), int(self.frame_height * 0.30)),
            (int(self.frame_width * 0.88), int(self.frame_height * 0.95)),
            (int(self.frame_width * 0.01), int(self.frame_height * 0.95))
            ],
            "right": [
            (int(self.frame_width * 0.70), int(self.frame_height * 0.35)),
            (int(self.frame_width * 0.98), int(self.frame_height * 0.35)),
            (int(self.frame_width * 1.00), int(self.frame_height * 0.74)),
            (int(self.frame_width * 0.56), int(self.frame_height * 0.53))
            ],
            "top": [
            (int(self.frame_width * 0.0), int(self.frame_height * 0.0)),
            (int(self.frame_width * 0.0), int(self.frame_height * 0.0)),
            (int(self.frame_width * 0.0), int(self.frame_height * 0.0)),
            (int(self.frame_width * 0.0), int(self.frame_height * 0.0))
            ]
        ...
        self.mpp = 0.44  # meters per pixel
        ...
        self.VTw_dict = {"top": 4.2, "bottom": 3.8, "right": 3.1}  # Green time
        self.VTL_dict = {"top": 4.7, "bottom": 3.1, "right": 3.2}  # Red time
        self.green_time = 0 
        self.yellow_time = 0
    ...
    def get_zones(self):
        zones = [
            ("Lane1", 
            (int(self.frame_width * 0.01), int(self.frame_height * 0.45)),
            (int(self.frame_width * 0.24), int(self.frame_height * 0.45)),
            (int(self.frame_width * 0.01), int(self.frame_height * 0.90)),
            (int(self.frame_width * 0.47), int(self.frame_height * 0.90)),
            (255, 255, 220)),  # Yellow

            ("Top_Lane1", 
            (int(self.frame_width * 0.01), int(self.frame_height * 0.40)),
            (int(self.frame_width * 0.21), int(self.frame_height * 0.40)),
            (int(self.frame_width * 0.01), int(self.frame_height * 0.45)),
            (int(self.frame_width * 0.24), int(self.frame_height * 0.45)),
            (128, 255, 255)),  # Yellow
            ...
            ("Lane2", 
            (int(self.frame_width * 0.24), int(self.frame_height * 0.45)),
            (int(self.frame_width * 0.44), int(self.frame_height * 0.45)),
            (int(self.frame_width * 0.47), int(self.frame_height * 0.90)),
            (int(self.frame_width * 0.88), int(self.frame_height * 0.90)),
            (255, 255, 0)),  # Cyan

            ("Top_Lane2", 
            (int(self.frame_width * 0.21), int(self.frame_height * 0.40)),
            (int(self.frame_width * 0.38), int(self.frame_height * 0.40)),
            (int(self.frame_width * 0.24), int(self.frame_height * 0.45)),
            (int(self.frame_width * 0.44), int(self.frame_height * 0.45)),
            (255, 255, 128)),  # Cyan

            ("Lane3_Right", 
            (int(self.frame_width * 0.70), int(self.frame_height * 0.45)),
            (int(self.frame_width * 0.99), int(self.frame_height * 0.45)),
            (int(self.frame_width * 0.70), int(self.frame_height * 0.53)),
            (int(self.frame_width * 1.00), int(self.frame_height * 0.74)),
            (128, 0, 128)),  # Purple

            ("Top_Lane3", 
            (int(self.frame_width * 0.75), int(self.frame_height * 0.30)),
            (int(self.frame_width * 0.98), int(self.frame_height * 0.30)),
            (int(self.frame_width * 0.65), int(self.frame_height * 0.45)),
            (int(self.frame_width * 1.00), int(self.frame_height * 0.45)),
            (128, 128, 128))
        ]
        return zones
    ...
    def is_in_lane1_left(self, center, zone):
        x, y = center
        _, tl, tr, bl, br, _ = zone

        # đơn giản hóa: kiểm tra theo hình chữ nhật nằm ngang
        if bl[0] <= x <= br[0] and tl[1] <= y <= bl[1]:
            self.logger.info("1. Lane1_Left")
            return True
        return False

    def is_in_top_lane1_left(self, center, zone):
        x, y = center
        _, tl, tr, bl, br, _ = zone

        # đơn giản hóa: kiểm tra theo hình chữ nhật nằm ngang
        if bl[0] <= x <= br[0] and tl[1] <= y <= bl[1]:
            self.logger.info("1. Top_Lane1_Left")
            return True
        return False

    def is_in_lane2_left(self, center, zone):
        x, y = center
        _, tl, tr, bl, br, _ = zone

        # đơn giản hóa: kiểm tra theo hình chữ nhật nằm ngang
        if tl[0] <= x <= br[0] and tl[1] <= y <= bl[1]:
            self.logger.info("2. Lane2_Left")
            return True
        return False

    def is_in_top_lane2_left(self, center, zone):
        x, y = center
        _, tl, tr, bl, br, _ = zone

        # đơn giản hóa: kiểm tra theo hình chữ nhật nằm ngang
        if tl[0] <= x <= br[0] and tl[1] <= y <= bl[1]:
            self.logger.info("2. Top_Lane2_Left")
            return True
        return False

    def is_in_lane3_right(self, center, zone):
        x, y = center
        _, tl, tr, bl, br, _ = zone
    ...
    def is_in_top_lane3_right(self, center, zone):
        x, y = center
        _, tl, tr, bl, br, _ = zone
    ...
    def calculate_speed_MPP(self, coordinates, bbox_list, fps, frame_width, frame_height, mpp, matrix):
        ...
        mpp_scaled = 0
        # Scale theo vùng
        if self.is_in_top_lane1_left(center, self.zones[1]) == True:
            block_parameter = self.adjust_speed_by_position(center[1], frame_height, 17.9)
            self.logger.info(f"BP: {block_parameter}")
                
            # scale theo chiều cao bbox
            mpp = 0.012
            mpp_scaled = mpp * ratio * block_parameter

        elif (self.is_in_top_lane2_left(center, self.zones[3]) == True):
            block_parameter = self.adjust_speed_by_position(center[1], frame_height, 15.9)
            self.logger.info(f"BP: {block_parameter}")

            mpp = 0.015
            mpp_scaled = mpp * ratio * block_parameter

        elif (self.is_in_top_lane3_right(center, self.zones[5]) == True):
            block_parameter = self.adjust_speed_by_position(center[1], frame_height, 17.5)
            self.logger.info(f"BP: {block_parameter}")

            mpp = 0.023
            mpp_scaled = mpp * ratio * block_parameter

        elif (self.is_in_lane3_right(center, self.zones[4]) == True):
            block_parameter = self.adjust_speed_by_position(center[1], frame_height, 7.8)
            self.logger.info(f"BP: {block_parameter}")

            mpp = 0.039
            mpp_scaled = mpp * ratio * block_parameter

        elif (self.is_in_lane1_left(center, self.zones[0]) == True):
            block_parameter = self.adjust_speed_by_position(center[1], frame_height, 13.5)
            self.logger.info(f"BP: {block_parameter}")

            mpp = 0.016
            mpp_scaled = mpp * ratio * block_parameter

        elif (self.is_in_lane2_left(center, self.zones[2]) == True):
            block_parameter = self.adjust_speed_by_position(center[1], frame_height, 12.9)
            self.logger.info(f"BP: {block_parameter}")
    ...
    def detect_cycle_direction(self, direction_counts_history):
        ...
        for counts in direction_counts_history:
            tb = counts.get('right', 0) + counts.get('bottom', 0)
            lr = counts.get('top', 0)
            tb_list.append(tb)
            lr_list.append(lr)
    ...
    def check_zone(self, center):
        if (self.is_in_top_lane1_left(center, self.zones[1]) == True) or (self.is_in_top_lane2_left(center, self.zones[3]) == True) or (self.is_in_lane1_left(center, self.zones[0]) == True) or (self.is_in_lane2_left(center, self.zones[2]) == True) or (self.is_in_top_lane3_right(center, self.zones[5]) == True) or (self.is_in_lane3_right(center, self.zones[4]) == True):
            return True
        else:
            return False
